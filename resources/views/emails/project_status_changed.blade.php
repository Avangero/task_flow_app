...
<head>
    ...
    <style>
        ...
        .project-info { background: #f8f9fa; padding: 20px; border-radius: 6px; margin: 20px 0; border-left: 4px solid #6f42c1; }
        .project-name { font-weight: 600; color: #2c3e50; margin-bottom: 8px; font-size: 18px; }
        .status-badge { display: inline-block; padding: 4px 12px; border-radius: 20px; font-size: 12px; font-weight: 600; text-transform: uppercase; }
        .status-old { background: #ffc107; color: #856404; }
        .status-new { background: #6f42c1; color: white; }
...
<body>
    <div class="container">
        <div class="header">
            <h2>{{ __('mail.project_status_changed.title') }}</h2>
        </div>
        <div class="content">
            <p>{{ __('mail.hello') }}, {{ $project->author->first_name }} {{ $project->author->last_name }}!</p>

            <div class="project-info">
                <div class="project-name">{{ $project->name }}</div>
                <p>{{ __('mail.project_status_changed.text', ['name' => $project->name, 'old' => $oldStatus->value, 'new' => $newStatus->value]) }}</p>
